...
- **Item-change trigger.** `Container(id).OnNext` / `OnPrevious` are true for one frame on every fixedlist move (cursor or scroll, see `CGUIFixedListContainer::SelectItem`). Conditional animations with `reversible="false"` on those drive the caption stagger, settle zoom, logo rise and callout pop. The caption text changes in the same frame as the move, so the old caption cannot fade out on an item change; it is hidden instantly and the new one staggers in after the 180 ms settle. On a row change the old caption does fade out (130 ms, 14 px left) because each row has its own caption instance.
...
- **Crossfade curve.** The fanart crossfade uses the image control's `<fadetime>700</fadetime>`, which is linear; Kodi has no tween option for it.

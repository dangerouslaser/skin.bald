...
**Settings** (Bald Settings, Playback, window 1119, the settings scaffold): Controls (close the controls after, time display, details on the controls), Pause (when paused, pause delay), Info overlay (artwork, plot), Down from the controls (guide, channels, playlist and episodes, bookmarks, cast and crew; cast is dimmed without TMDb Helper). Names: `Bald.OSD.AutoClose`, `PauseMode`, `PauseDelay`, `TimeDisplay`, `DetailsOnControls`, `HideInfoArt`, `HideInfoPlot`, `NoGuidePanel`, `NoChannelsPanel`, `NoPlaylistPanel`, `NoBookmarksPanel`, `NoCastPanel`; unset is the default.

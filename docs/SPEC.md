...
### 5.16 Video OSD (implemented 2026-09-26, not yet run in Kodi)

Not in the prototype. Composition and behaviour follow Arctic Fuse 3's OSD (the brief the user approved on 2026-09-26), rewritten in Bald's own XML, fonts, palette and motion; no Arctic Fuse code or text is used. The video OSD lives in `Includes_Bald_OSD.xml`; the track pieces, the music OSD, PlayerControls and the small player popups stay in `Includes_Bald_Playback.xml`. Replaces "Playback (proposed)" for the video OSD, the seek bar, the pause header and the info overlay.

**Layers.** From the bottom: the seek layer (`DialogSeekBar.xml`, zorder 0); `VideoOSD.xml` and the companion panels 1120-1124 (zorder 1); the down-chain panels (1125, `VideoOSDBookmarks.xml`, 1126, the Live TV lists); the info overlay (1127) and details on the controls (1128), which open by their visibility conditions and take no focus.

**Seek layer** (`DialogSeekBar.xml`).
- Bottom gradient: the info dialog's `scrim_info_b` in `field`, short (from y 420) with the bar alone, tall (the default stretch, to about y 542) with the OSD, a panel or the info overlay; under a bottom panel the video also dims to `field60`. Home's clock (132) at 1440,104 with "Ends at" or the timeshift position under it, over the flipped scrim.
- Status line at 96,864: Info meta 24, `ink`: "Playing", "Paused", "Seeking +30s", "Fast forward 4x", "Rewind 2x", "Cache 42%", "Play speed 1.10x", "Seeking 01:20:00" (typed), "Channel 12" (typed). Right-aligned to 1824, Caption meta 70%: the chapter ("Chapter 3 / 12 · name"), for Live TV the channel and programme times, or "Currently recording".
- Bar at y 930: a 4 px line from x 264 to 1656 in 28% `ink`, the cache line in 45%, the played part in `accent`; chapter and scene ticks as `field` gaps, cut ticks and edit-list ranges at 60%, bookmarks in `accent`. Live TV: the timeshift buffer in 45% and the programme so far in `accent`.
- Seek-preview nib: sliders 401 (files), 402 (EPG event with the seek offset) and 403 (timeshift without EPG), 16 px `accent`, shown while `Player.Seeking` and faded out 300 ms after (Hidden animation delay). Kodi positions them (`CGUIDialogSeekBar`: 401 follows the seek target while a seek is pending).
- Times either side of the bar at y 918 (Caption meta 20): left `ink`, right 70%. Time display setting: Elapsed/Total (default), Remaining/Total, Elapsed/Remaining (remaining shown with a minus). While seeking the left time is the seek target.
- Lift: the status line, times and bar slide up 136 px (480 MOVE) while the OSD or a companion panel is up (bar at y 794), and 496 px (bar at y 434) while a bottom panel is up. The Live TV lists fade the block out. The info overlay replaces it. Music keeps its title block over the status line.
- Shows when: paused (unless pause behaviour Hide has run), caching, seeking or just seeked, FF/RW, a typed seek time or channel, `Player.ShowTime`/`ShowInfo`, the info overlay, the OSD or any OSD panel, Up Next. Hidden under DialogSlider, player process info and games.

**Button row** (`VideoOSD.xml`). 80 x 96 buttons at y 836, 8 px apart: transport from x 96 (play/pause 602, back 600, forward 607, stop 603), tools ending at x 1824 (disc menu 804, teletext 70045 and 3D 70047 when they apply, then information 70043, audio 70060, subtitles 70061, video 70062). Estuary's 48 px icons at 16,12 in 70% `ink`; focused, the icon turns `field` on a 72 px `accent` disc (`bald/osd_disc.png`) with the tile pop (1.05, 280 POP). Audio and subtitles carry the language code under the disc (Flag 14; subtitles only while on). Each button is a radiobutton directly in its row with its own `<visible>`. Left from the first transport button wraps to the last tool and back.
- Back: previous chapter, else previous playlist item, else a big step back. Forward: next chapter, else next playlist item, else a big step. Live TV: back shows the guide icon and opens the TV guide, forward shows the record icon and toggles recording (a dot under it while recording). Kodi's radio textures take no `$VAR`, so each button carries both icons and picks one with `<selected>`.
- Up from a transport button reaches an invisible seek slider over the lifted bar (87 `seek`, 88 `pvr.seek` for Live TV), marked by the accent dot left of the bar and a 16 px `accent` nib (the sliders' bar is the transparent, nib-sized `slider_clear.png`, so Kodi draws the nib 1:1); Down returns to the row.
- Initial focus: play/pause; with pause unavailable (Live TV) the onload moves it to back (600).
- Hints on y 954: the focused button's name, then "Up to choose" (audio, subtitles, video), "Down for more" (a chain panel exists), "Up to seek" or "Back to close".
- The OSD opens with 380 FADE and a 40 px rise over 480 MOVE and closes in 160; the row fades (160) while a chain panel, DialogSlider or process info is up.

**Companion panels** (focus-to-preview). Focusing information, audio, subtitles or video opens its panel (1120-1123); focusing forward opens the next item card (1124). Each panel window repeats the row: its own button real, the others stand-ins with the same ids and visibility that close the panel at once and focus the OSD's button, so Left and Right carry on along the row. Panels close without an animation; their content fades in (300 after 80, 24 px rise).
- Panel surface: an opaque `field` panel ending at x 1824, bottom at y 712, a 72 px heading band in 10% `ink` (Section 22), 64 px rows: 70% `ink`, full `ink` on focus with a 10% band and the 10 px `accent` dot.
- Information (1120): the details block (1128) shows at the top; Select closes the OSD and opens the info overlay.
- Audio and subtitles (1121, 1122): the streams from script.skinvariables (`get_player_streams`, `stream_type=audio|subtitle`), sized to the row count (up to six rows visible). Each row: language code, name, "forced · default · original · codec · channels", a hearing-impaired mark, and an `accent` check on the stream in use (for subtitles, only while they are on). Select switches (`set_player_audiostream`, `set_player_subtitle`); Select on the current subtitle while subtitles are on turns them off. Up from the button enters the list; Down, Left or Right leaves it. Select on the button opens Kodi's audio or subtitle settings.
- Video (1123): a Player process info row with the codec flags (resolution, codec, HDR, aspect, audio codec, channels) that opens process info, and a Play speed row: Left and Right change the tempo when tempo is on, otherwise rewind or fast forward (either resumes a paused video), over a scale (0.8-1.5, or 32× rewind to 32× fast forward) with an `accent` nib sliding to the current step. Select on the button opens Kodi's video settings.
- Next item card (1124): at 96,96, 384 x 216 art with "Up next" (Accent 18), the title (Info meta 24) and a meta line, when no chapter is left and either a playlist item follows (`VideoPlayer.offset(1)`) or, in the last ten minutes of a library episode, the next episode of the season exists (a one-item videodb query, episode greater than the current one). Forward then plays that episode.
...
- Bookmarks and chapters (`VideoOSDBookmarks.xml`, Kodi's 2, 3, 4 and 11): the pills at y 528 and 288 x 162 thumbnails at y 612.
- Cast and crew (1126): TMDb Helper's cast list (112 px circles, name and role; Bald_CastItem); Down switches to the crew, Up back to the cast; the tray shows the person and their biography, Select opens it in the text viewer.
- The Live TV guide and channel lists keep their own layout (5.18); from the guide, Right (or Down past the last programme) opens the channels.

**Auto-close.** Close the controls after: never (default), 4, 6, 8, 10 or 15 s. Each button arms an alarm on focus and click and cancels it on unfocus; DialogSettings (the OSD audio, subtitle and video settings), DialogSlider, DialogSubtitles and the OSD's own popups suspend it. Replaces Estuary's `autoclosevideoosd` timer; an old Estuary setting is carried over once at startup.

**Up Next** (service.upnext, only when installed): `script-upnext-upnext.xml` and `script-upnext-stillwatching.xml` (and the `-simple` variants) as a bottom panel: art with the countdown (3014, `accent`) under it, show, episode and "Ends at", plot, and "Watch now"/"Continue watching" (3012) and "Close" (3013, which the add-on relabels) pills.

**Settings** (Bald Settings, Playback, window 1119, the settings scaffold): Controls (close the controls after, time display, and clearlogo, media flags and plot on the controls, each switched on its own and shown together in the top-left details block), Pause (when paused, pause delay), Info overlay (artwork, plot), Down from the controls (guide, channels, playlist and episodes, bookmarks, cast and crew; cast is dimmed without TMDb Helper). Names: `Bald.OSD.AutoClose`, `PauseMode`, `PauseDelay`, `TimeDisplay`, `DetailsOnControls`, `HideInfoArt`, `HideInfoPlot`, `NoGuidePanel`, `NoChannelsPanel`, `NoPlaylistPanel`, `NoBookmarksPanel`, `NoCastPanel`; unset is the default.

**Other playback windows** (from "Playback (proposed)", unchanged):
- **OSD settings** (`Custom_1101_SettingsDialog.xml`, now only 3D from the video OSD, and the music OSD's settings): a right-hand drawer over the dimmed player; the settings scaffold's rows in a 672 px column ending at x 1824.
- **Small player popups** (`DialogSlider.xml`, `Custom_1110_TempoControl.xml`): an opaque `field` panel, 840 wide, centred at y 96.
- **Music OSD** (`MusicOSD.xml`): its 64 x 84 icon buttons at y 818 under the seek layer's lifted bar, its title block over the status line.
- **Player controls** (`PlayerControls.xml`), **subtitle search** (`DialogSubtitles.xml`) and **buffering** (`VideoFullScreen.xml`, with the view-mode lines 10-12 at the top left) as before.
- **Icons.** Bald has no icon set: the OSD tints Estuary's `osd/fullscreen/buttons` icons.

...
- Letter mode adds a quiet label and accent-colored current-letter indicator beneath the posters at y866. Left/Right uses Kodi's native previous/next-letter groups, updating the posters and preview without filtering or reloading the library. Up, Select or Back returns to the selected poster (Select does not open details until back in normal browsing). Number groups display 0–9. Native title sorting/article handling and filtered results are respected. If another sort is selected, prompt for Title sort instead of changing the user's sorting preference. Mode-specific footer hints use the shared 20 px dot spacing.

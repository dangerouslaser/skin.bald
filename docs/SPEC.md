...
- Back: previous chapter, else previous playlist item, else a big step back. Forward: next chapter, else next playlist item, else a big step. Live TV: back opens the TV guide, forward toggles recording (the icon is `accent` while recording).
- Up from a transport button reaches an invisible seek slider over the lifted bar (87 `seek`, 88 `pvr.seek` for Live TV), marked by the accent dot left of the bar and a 16 px `accent` nib; Down returns to the row.
